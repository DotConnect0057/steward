...
func ApplyConfigWithProgress(config *common.Config) (*common.Config) {
    ...
    for _, host := range config.Hosts {
		...
		commonExternalApps := 0
		hostExternalApps := 0

		for _, app := range config.Common.Application.External {
			commonExternalApps += len(app.Packages)
		}
    ...
    for i, host := range config.Hosts {
        ...
        go func(taskIndex int, host common.Host) {
            ...
            for pkg, ver := range config.Common.Application.Core {
                fullPkgName := ""
				if ver != "" {
			...
			for pkg, ver := range host.Application.Core {
				fullPkgName := ""
				if ver != "" {
					fullPkgName = fmt.Sprintf("%s=%s", pkg, ver)
				} else {
			...
			for _, app := range config.Common.Application.External {
				...
				if app.GPGKeyURL != "" {
					...
					if err != nil {
						mu.Lock()
						logger.Errorf("Error installing GPG key %s on host %s: %v", app.Name, host.Host, err)
						tasks[taskIndex].Status = "Error"
						mu.Unlock()
						return
					}
					mu.Lock()
					logger.Infof("Installed GPG key %s on host %s", app.Name, host.Host)
					mu.Unlock()
				}

				// Install repo skip if empty
				if app.Repo != "" {
					err := aptman.AddRepository(host.Password, app.Name, app.Repo)
					if err != nil {
						mu.Lock()
						logger.Errorf("Error adding repo %s on host %s: %v", app.Name, host.Host, err)
						tasks[taskIndex].Status = "Error"
						mu.Unlock()
						return
					}
					mu.Lock()
					logger.Infof("Added repo %s on host %s", app.Name, host.Host)
					mu.Unlock()
				}

				// Install packages
				for pkg, ver := range app.Packages {
					fullPkgName := ""
					if ver != "" {
						fullPkgName = fmt.Sprintf("%s=%s", pkg, ver)
					} else {
